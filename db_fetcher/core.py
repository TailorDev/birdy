...
Usage:
  core.py [-v | -d ] [-f] [PATH] [DIRECTORY] [--nocache]
  core.py (-h | --help)
...
from . import fetcher, config, converter
...
def validation(args):
    ...
    schema = Schema({
        ...
        Optional('m'): And(
            Use(int),
            lambda n: 1 <= n <= 50,
            error="'max_per_family' should be an integer between 1 and 50"),
